...
Dollar (DXY): ${mac.dxy ? `${mac.dxy.last.toFixed(1)} [1M RS: ${mac.dxy.rs1m > 0 ? '+' : ''}${mac.dxy.rs1m.toFixed(2)}% — internal, not user-visible, do not cite this percentage]` : 'N/A'}
...
export async function POST(req: Request) {
  try {
    ...
    const message = await client.messages.create({
      ...
      max_tokens: 2500,
      system: 'You are a senior FinOps market analyst writing for Bloomberg terminal users. Your style: lead with the story, support with numbers. One clear takeaway per paragraph. Write "Infrastructure is hot, software is not" not "SaaS cohort at 6-8x NTM P/S reflects compression." Be direct, confident, and specific. GROUNDING RULE: Use ONLY the data and estimates provided in the user message. Do not invent percentages, industry benchmarks, or statistics not present in that context. If a specific number is not in the data you were given, use qualitative language instead ("compressed," "elevated," "tightening"). Do not cite named industry reports, analysts, or vendor data sources unless explicitly provided in the context. VERIFIABILITY RULE: This applies to EVERY output field — morningBrief paragraphs, finopsSignals, commitmentWindows reasons, riskAlerts messages, sectorInsights, and all captions. When citing numeric values, only use numbers that appear in the user-visible dashboard sections (Market Status, Market Internals, Macro Context, Sectors, Cloud Valuations, Hyperscaler CapEx, Tech Concentration, Momentum Leaders/Laggards, AI Compute Commitments). Do not cite numeric scores or percentages that exist only in your internal data context but are not displayed to users — any field annotated [internal] or [not user-visible] must not be quoted as a number anywhere in your output. Specifically: do not cite 1-month RS percentages for DXY, GLD, IWM, or QQQ individually — the dashboard shows their price and daily change only (for DXY/GLD/USO) or direction and spread only (for IWM/QQQ). Use qualitative language instead: "the dollar is weakening," "gold is rising," "small caps are lagging" — never "dollar down 9% in a month" or similar uncitable percentages. You MUST respond with ONLY valid JSON — no markdown, no code blocks, no preamble.',
